...
if (!kullanıcı) return message.channel.send(new MessageEmbed().setDescription(`Odasına gitmek istedğiniz kullanıcıyı belirtmeniz gerekir @mithra/İD gibi`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('RANDOM').setTimestamp()).then(x => x.delete({timeout: 5000}));
